...
<title>REXWIFT- admin log in</title>
<?php
	session_start();
...
<div class="logsign">
			<form method="post">
			<h2 align="center" style="font-family:Impact, Charcoal, sans-serif; color:#00264d">ADMIN LOGIN</h2><br>
				<table>
					<tr>
						<td>Email Address:</td>
					...
					<tr>
						...
						<center><input type="submit" name="login" value="Login" style="font-size:20px;"></center><br>
			...
			<?php
				if(isset($_POST['login'])){
					...
					$query = "SELECT * FROM radmin WHERE email = '$email' AND pass = '$pass'";
					$rs = $conn->query($query);
					$num = $rs->num_rows;
					$rows = $rs->fetch_assoc();
					if($num > 0){
						...
						echo "<script type = \"text/javascript\">
									...
									window.location = (\"rexwift_administrator.php\")
									</script>";
					} else{
						echo "<script type = \"text/javascript\">
